...
## 4 suggested factors
fit <- factanal(Predictors, 4, rotation = "varimax")
print(fit, digits=2, cutoff=.4, sort=TRUE)

fit <- factanal(Predictors, 4, rotation = "promax")
print(fit, digits=2, cutoff=.4, sort=TRUE)
...
F_dat <- dat %>% 
  mutate(F1 = KNOWWHAT + SAFETYWK + RESPECT + TRUSTMAN + MYSKILLS ,
        F2 = WKDECIDE, 
         F3 = WORKFAST + LEARNNEW + WORKDIFF,
         F4 = LOTOFSAY + WKFREEDM) %>% 
  dplyr::select( F1:F4, satisfaction, JOBSECOK, WKPRAISE, FAIREARN)

set.seed(123)
sample = sample.split(F_dat,SplitRatio = 0.75) # splits the data in the ratio mentioned in SplitRatio. After splitting marks these rows as logical TRUE and the the remaining are marked as logical FALSE
train = subset(F_dat,sample ==TRUE) # creates a training dataset named train1 with rows which are marked as TRUE
test = subset(F_dat, sample==FALSE)
...
F_dat %>% 
  gather(F1:F4, JOBSECOK, FAIREARN, key = "factor", value ="score") %>% 
  ggplot(aes(x = score, y = satisfaction, color = factor(WKPRAISE)))+
  geom_point(alpha = 0.01)+
  geom_smooth(method = "lm")+
  facet_wrap(~factor, scales = "free", nrow = 1 )+
  theme_classic()+
  labs(x = " Factor Score", y = "Satisfaction Score", title = "Work Factors and Life Satisfaction Scores", color = "Work Praise")+
  theme(legend.position = "bottom")

ggsave("FactorsAndSigVarGraph.jpeg")  
